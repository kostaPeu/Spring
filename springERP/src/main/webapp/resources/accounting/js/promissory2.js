...
function handler(data) {
	...
	$.each(data, function(index, list) {
		html += '<tr class="getValue"><td class = "getcustomer_id">'+list.customer_id+'</td>';
		html += '<td class="getcustomer_name">'+list.customer_name+'</td>';
...
$('.note_id_search').click(function() {
	$('.n_searchTable').empty();
	$('.search_note_id').val('');
	var url = "/Search/promissory/Payment/list";
	$.ajax({
		url : url,
		dataType : 'json',
		success: handler
...
$('.noteIds').click(function() {	
	...
	var url = "/Search/promissory/Payment/Idlist?note_id="+note_id;
	$('.n_searchTable').empty();
	$.ajax({
		url : url,
		dateType : 'json',
...
//어음결제 출력 ajax 이벤트 처리
function handler(data) {
	var html = '<tr><th>어음 번호</th><th>출금계좌번호</th><th>부서아이디</th><th>프로젝트</th><th>금액</th><th>비고</th></tr>';
	$.each(data, function(index, list) {
		html += '<tr class="getValue"><td class = "getnode_id">'+list.note_id+'</td>';
		html += '<td class="getcustomer_name">'+list.account_number+'</td>';
